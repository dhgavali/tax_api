...
def get_license(db: Session, refno: int):
    return db.query(models.License).filter(models.License.refno == refno).first()
...
def create_license(db: Session, license: schemas.LicenseCreate):
    db_license = models.License(
        ...
        licenseCategoryName=license.licenseCategoryName,
    )
    db.add(db_license)
    db.commit()
